    ...
    <head>
        ...
        <style>
            @page {
                margin-top:  {{ ($clinic->pres_margin_top)*10 ?? 40 }}px;
                margin-bottom:  {{ ($clinic->pres_margin_bottom)*10 ?? 40 }}px;
                margin-left: 10mm;
                margin-right: 10mm;
            }
            body {
    ...
    <p>{{ $list_item['name'] }} (Since: {{ $list_item['since'] ?? '-' }} | Severity: {{ $list_item['severity'] ?? '-' }} | Notes: {{ $list_item['note'] ?? '' }})</p>
    @endforeach
    @endif
